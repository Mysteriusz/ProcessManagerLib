#pragma once

enum GPU_GIF_FLAGS {
    GPU_GIF_DX_SUPPORT = 0x00000001,
    GPU_GIF_VRAM_USAGE = 0x00000002,
    GPU_GIF_VRAM_SIZE = 0x00000004,
    GPU_GIF_UTILIZATION = 0x00000008,
    GPU_GIF_VIDEO_ENCODE = 0x00000010,
    GPU_GIF_VIDEO_DECODE = 0x00000020,
    GPU_GIF_COPY = 0x00000040,
    GPU_GIF_MAX_RES = 0x00000080,
    GPU_GIF_MIN_RES = 0x00000100,
    GPU_GIF_MODEL_INFO = 0x00000200,
};

enum GPU_MIF_FLAGS {
    GPU_MIF_NAME = 0x00000001,
    GPU_MIF_VENDOR = 0x00000002,
    GPU_MIF_DRIVER_NAME = 0x00000004,
    GPU_MIF_DRIVER_VERSION = 0x00000008,
    GPU_MIF_ID = 0x00000010,
    GPU_MIF_REVISION = 0x00000020,
};

enum GPU_UIF_FLAGS {
    GPU_UIF_UTILIZATION = 0x00000001,
    GPU_UIF_VIDEO_ENCODE = 0x00000002,
    GPU_UIF_VIDEO_DECODE = 0x00000004,
    GPU_UIF_COPY = 0x00000008,
};

enum GPU_PIF_FLAGS {
    GPU_PIF_BUS_ID = 0x00000001,
    GPU_PIF_LEGACY_BUS_ID = 0x00000002,
    GPU_PIF_BUS = 0x00000004,
    GPU_PIF_DOMAIN = 0x00000008,
    GPU_PIF_DEVICE_ID = 0x00000010,
    GPU_PIF_PCI_DEVICE_ID = 0x00000020,
    GPU_PIF_SUBSYS_DEVICE_ID = 0x00000040,
};

enum GPU_RIF_FLAGS {
    GPU_RIF_WIDTH = 0x00000001,
    GPU_RIF_HEIGHT = 0x00000002,
};